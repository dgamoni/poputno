...
//    function validateEventForm(){
        jQuery('.tribe-events-community-footer1 .events-community-submit').on('click',function(e){
            jQuery('.validate-msg').remove();
            jQuery('.events-community-post-title input').css({
                'border-color':'#ddd'
            });
            if(jQuery('.events-community-post-title input').val()==""){
                e.preventDefault();

                jQuery('.events-community-post-title input').css({
                    'border-color':'rgba(220, 69, 54, 0.57)'
                })
                jQuery('.events-community-post-title input').after('<p class="validate-msg">Поле \"Заголовок события\" не может быть пустым</p>');
                console.log('empty');
                console.log(jQuery('#tcepostcontent_ifr').contents().find('#tinymce p').html());
            }
            if(jQuery('#tcepostcontent_ifr').contents().find('#tinymce p').html()=='<br data-mce-bogus="1">'){
                e.preventDefault();
                jQuery('#wp-tcepostcontent-wrap').after('<p class="validate-msg">Поле \"Описание события\" не может быть пустым</p>');
                console.log('empty2');
            }
